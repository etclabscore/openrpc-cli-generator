...
func GenerateCLI(spec1 *types.OpenRPCSpec1, programName string) error {
	...
	targetPath =  filepath.Join(targetPath, "go")
	...
	err = generate.WriteFile(box, "server", filepath.Join(targetPath, programName, "cmd"), spec1)
	if err != nil {
		return fmt.Errorf("server: %v", err)
	}

	err = generate.WriteFile(box, "types", filepath.Join(targetPath, programName, "cmd"), spec1)
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("cli_cmd: %v", err)
	}
